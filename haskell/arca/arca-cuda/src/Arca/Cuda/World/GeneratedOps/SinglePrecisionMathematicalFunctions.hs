...
cyl_bessel_i0f = opify (Op "cyl_bessel_i0f" :: Op (OneArg F) (OneArg F))
...
nearbyintf = opify (Op "nearbyintf" :: Op (OneArg F) (OneArg F))
...
nextafterf = opify (Op "nextafterf" :: Op (TwoArg F F) (OneArg F))
...
norm3df = opify (Op "norm3df" :: Op (ThreeArg F F F) (OneArg F))
...
norm4df = opify (Op "norm4df" :: Op (FourArg F F F F) (OneArg F))

-- | Calculate the standard normal cumulative distribution function.
normcdff :: (HasIdentifier w, HasStackLens w F) => PartialStackOp w
normcdff = opify (Op "normcdff" :: Op (OneArg F) (OneArg F))

-- | Calculate the inverse of the standard normal cumulative distribution function.
normcdfinvf :: (HasIdentifier w, HasStackLens w F) => PartialStackOp w
normcdfinvf = opify (Op "normcdfinvf" :: Op (OneArg F) (OneArg F))
...
powf = opify (Op "powf" :: Op (TwoArg F F) (OneArg F))

-- | Calculate reciprocal cube root function.
rcbrtf :: (HasIdentifier w, HasStackLens w F) => PartialStackOp w
rcbrtf = opify (Op "rcbrtf" :: Op (OneArg F) (OneArg F))

-- | Compute single-precision floating-point remainder.
remainderf :: (HasIdentifier w, HasStackLens w F) => PartialStackOp w
remainderf = opify (Op "remainderf" :: Op (TwoArg F F) (OneArg F))
...
rhypotf = opify (Op "rhypotf" :: Op (TwoArg F F) (OneArg F))

-- | Round input to nearest integer value in floating-point.
rintf :: (HasIdentifier w, HasStackLens w F) => PartialStackOp w
rintf = opify (Op "rintf" :: Op (OneArg F) (OneArg F))
...
rnorm3df = opify (Op "rnorm3df" :: Op (ThreeArg F F F) (OneArg F))
...
rnorm4df = opify (Op "rnorm4df" :: Op (FourArg F F F F) (OneArg F))
