...
__clzll = opify (Op "__clzll" :: Op (OneArg L) (OneArg I))
...
__ffs = opify (Op "__ffs" :: Op (OneArg I) (OneArg I))
...
__ffsll = opify (Op "__ffsll" :: Op (OneArg L) (OneArg I))
...
__funnelshift_l = opify (Op "__funnelshift_l" :: Op (ThreeArg UI UI UI) (OneArg UI))
...
__funnelshift_lc = opify (Op "__funnelshift_lc" :: Op (ThreeArg UI UI UI) (OneArg UI))
...
__funnelshift_r = opify (Op "__funnelshift_r" :: Op (ThreeArg UI UI UI) (OneArg UI))
...
__funnelshift_rc = opify (Op "__funnelshift_rc" :: Op (ThreeArg UI UI UI) (OneArg UI))
...
__hadd = opify (Op "__hadd" :: Op (TwoArg I I) (OneArg I))
...
__mul24 = opify (Op "__mul24" :: Op (TwoArg I I) (OneArg I))
...
__mul64hi = opify (Op "__mul64hi" :: Op (TwoArg L L) (OneArg L))
...
__uhadd = opify (Op "__uhadd" :: Op (TwoArg UI UI) (OneArg UI))
...
__umul24 = opify (Op "__umul24" :: Op (TwoArg UI UI) (OneArg UI))
...
__umul64hi = opify (Op "__umul64hi" :: Op (TwoArg UL UL) (OneArg UL))
...
__umulhi = opify (Op "__umulhi" :: Op (TwoArg UI UI) (OneArg UI))
...
__urhadd = opify (Op "__urhadd" :: Op (TwoArg UI UI) (OneArg UI))

-- | Calculate | x − y | + z , the sum of absolute difference.
__usad :: (HasIdentifier w, HasStackLens w UI) => PartialStackOp w
__usad = opify (Op "__usad" :: Op (ThreeArg UI UI UI) (OneArg UI))
