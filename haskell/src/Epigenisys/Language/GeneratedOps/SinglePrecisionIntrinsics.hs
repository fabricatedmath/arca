...
__cosf :: (HasStackLens w F) => State w ()
...
__exp10f :: (HasStackLens w F) => State w ()
...
__expf :: (HasStackLens w F) => State w ()
__expf = opify (Op False "__expf" :: Op (OneArg F) (OneArg F))

-- | Add two floating point values in round-down mode.
__fadd_rd :: (HasStackLens w F) => State w ()
__fadd_rd = opify (Op False "__fadd_rd" :: Op (OneArg F) (TwoArg F F))

-- | Add two floating point values in round-to-nearest-even mode.
__fadd_rn :: (HasStackLens w F) => State w ()
__fadd_rn = opify (Op False "__fadd_rn" :: Op (OneArg F) (TwoArg F F))

-- | Add two floating point values in round-up mode.
__fadd_ru :: (HasStackLens w F) => State w ()
__fadd_ru = opify (Op False "__fadd_ru" :: Op (OneArg F) (TwoArg F F))

-- | Add two floating point values in round-towards-zero mode.
__fadd_rz :: (HasStackLens w F) => State w ()
__fadd_rz = opify (Op False "__fadd_rz" :: Op (OneArg F) (TwoArg F F))

-- | Divide two floating point values in round-down mode.
__fdiv_rd :: (HasStackLens w F) => State w ()
__fdiv_rd = opify (Op False "__fdiv_rd" :: Op (OneArg F) (TwoArg F F))

-- | Divide two floating point values in round-to-nearest-even mode.
__fdiv_rn :: (HasStackLens w F) => State w ()
__fdiv_rn = opify (Op False "__fdiv_rn" :: Op (OneArg F) (TwoArg F F))

-- | Divide two floating point values in round-up mode.
__fdiv_ru :: (HasStackLens w F) => State w ()
__fdiv_ru = opify (Op False "__fdiv_ru" :: Op (OneArg F) (TwoArg F F))

-- | Divide two floating point values in round-towards-zero mode.
__fdiv_rz :: (HasStackLens w F) => State w ()
__fdiv_rz = opify (Op False "__fdiv_rz" :: Op (OneArg F) (TwoArg F F))

-- | Calculate the fast approximate division of the input arguments.
__fdividef :: (HasStackLens w F) => State w ()
__fdividef = opify (Op False "__fdividef" :: Op (OneArg F) (TwoArg F F))

-- | Compute x × y + z as a single operation, in round-down mode.
__fmaf_rd :: (HasStackLens w F) => State w ()
__fmaf_rd = opify (Op False "__fmaf_rd" :: Op (OneArg F) (ThreeArg F F F))

-- | Compute x × y + z as a single operation, in round-to-nearest-even mode.
__fmaf_rn :: (HasStackLens w F) => State w ()
__fmaf_rn = opify (Op False "__fmaf_rn" :: Op (OneArg F) (ThreeArg F F F))

-- | Compute x × y + z as a single operation, in round-up mode.
__fmaf_ru :: (HasStackLens w F) => State w ()
__fmaf_ru = opify (Op False "__fmaf_ru" :: Op (OneArg F) (ThreeArg F F F))

-- | Compute x × y + z as a single operation, in round-towards-zero mode.
__fmaf_rz :: (HasStackLens w F) => State w ()
__fmaf_rz = opify (Op False "__fmaf_rz" :: Op (OneArg F) (ThreeArg F F F))

-- | Multiply two floating point values in round-down mode.
__fmul_rd :: (HasStackLens w F) => State w ()
__fmul_rd = opify (Op False "__fmul_rd" :: Op (OneArg F) (TwoArg F F))

-- | Multiply two floating point values in round-to-nearest-even mode.
__fmul_rn :: (HasStackLens w F) => State w ()
__fmul_rn = opify (Op False "__fmul_rn" :: Op (OneArg F) (TwoArg F F))

-- | Multiply two floating point values in round-up mode.
__fmul_ru :: (HasStackLens w F) => State w ()
__fmul_ru = opify (Op False "__fmul_ru" :: Op (OneArg F) (TwoArg F F))

-- | Multiply two floating point values in round-towards-zero mode.
__fmul_rz :: (HasStackLens w F) => State w ()
__fmul_rz = opify (Op False "__fmul_rz" :: Op (OneArg F) (TwoArg F F))

-- | Compute 1 x in round-down mode.
__frcp_rd :: (HasStackLens w F) => State w ()
__frcp_rd = opify (Op False "__frcp_rd" :: Op (OneArg F) (OneArg F))

-- | Compute 1 x in round-to-nearest-even mode.
__frcp_rn :: (HasStackLens w F) => State w ()
__frcp_rn = opify (Op False "__frcp_rn" :: Op (OneArg F) (OneArg F))

-- | Compute 1 x in round-up mode.
__frcp_ru :: (HasStackLens w F) => State w ()
__frcp_ru = opify (Op False "__frcp_ru" :: Op (OneArg F) (OneArg F))

-- | Compute 1 x in round-towards-zero mode.
__frcp_rz :: (HasStackLens w F) => State w ()
__frcp_rz = opify (Op False "__frcp_rz" :: Op (OneArg F) (OneArg F))

-- | Compute 1 / x in round-to-nearest-even mode.
__frsqrt_rn :: (HasStackLens w F) => State w ()
__frsqrt_rn = opify (Op False "__frsqrt_rn" :: Op (OneArg F) (OneArg F))

-- | Compute x in round-down mode.
__fsqrt_rd :: (HasStackLens w F) => State w ()
__fsqrt_rd = opify (Op False "__fsqrt_rd" :: Op (OneArg F) (OneArg F))

-- | Compute x in round-to-nearest-even mode.
__fsqrt_rn :: (HasStackLens w F) => State w ()
__fsqrt_rn = opify (Op False "__fsqrt_rn" :: Op (OneArg F) (OneArg F))

-- | Compute x in round-up mode.
__fsqrt_ru :: (HasStackLens w F) => State w ()
__fsqrt_ru = opify (Op False "__fsqrt_ru" :: Op (OneArg F) (OneArg F))

-- | Compute x in round-towards-zero mode.
__fsqrt_rz :: (HasStackLens w F) => State w ()
__fsqrt_rz = opify (Op False "__fsqrt_rz" :: Op (OneArg F) (OneArg F))

-- | Subtract two floating point values in round-down mode.
__fsub_rd :: (HasStackLens w F) => State w ()
__fsub_rd = opify (Op False "__fsub_rd" :: Op (OneArg F) (TwoArg F F))

-- | Subtract two floating point values in round-to-nearest-even mode.
__fsub_rn :: (HasStackLens w F) => State w ()
__fsub_rn = opify (Op False "__fsub_rn" :: Op (OneArg F) (TwoArg F F))

-- | Subtract two floating point values in round-up mode.
__fsub_ru :: (HasStackLens w F) => State w ()
__fsub_ru = opify (Op False "__fsub_ru" :: Op (OneArg F) (TwoArg F F))

-- | Subtract two floating point values in round-towards-zero mode.
__fsub_rz :: (HasStackLens w F) => State w ()
__fsub_rz = opify (Op False "__fsub_rz" :: Op (OneArg F) (TwoArg F F))

-- | Calculate the fast approximate base 10 logarithm of the input argument.
__log10f :: (HasStackLens w F) => State w ()
