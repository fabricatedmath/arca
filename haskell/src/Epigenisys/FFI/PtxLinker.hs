module Epigenisys.FFI.PtxLinker where
...
ptxLink :: PtxLinker -> Text -> Text -> IO CUResult
ptxLink ptxLinker ptx funcName = 
    (cuResultMap IntMap.!) <$>
    withForeignPtr (_ptxLinker ptxLinker) (\ptr ->
        T.withCStringLen ptx (\(ptxStr, ptxStrLen) ->
            T.withCStringLen funcName (\(funcNameStr, funcNameStrLen) ->
                c_ptxLinkerLink ptr ptxStr ptxStrLen funcNameStr funcNameStrLen
            )   
        )
    )

ptxRun :: PtxLinker -> Int -> Int -> IO CUResult
ptxRun ptxLinker numBlocks numThreads = 
    (cuResultMap IntMap.!) <$>
    withForeignPtr (_ptxLinker ptxLinker) (\ptr -> 
        c_ptxLinkerRun ptr numBlocks numThreads
    )

cuResultMap :: IntMap CUResult
cuResultMap = IntMap.fromList cuResultList      
    where
        cuResultList :: [(Int,CUResult)]
        cuResultList = 
            [ (0, CUDA_SUCCESS)
