 ...
 */
 ...
 int main(int argc, char *argv[])
 {
    ...
    const unsigned int numBlocks = blockCount;
    const unsigned int numThreads = threadsPerBlock;
     ...
     /* Setup prng states */
     if (useMRG) {
         setup_kernel<<<numBlocks, numThreads>>>(devMRGStates);
     }else if(usePHILOX)
     {
         setup_kernel<<<numBlocks, numThreads>>>(devPHILOXStates);
     }else {
         setup_kernel<<<numBlocks, numThreads>>>(devStates);
     }
     ...
     for(i = 0; i < 50; i++) {
         if (useMRG) {
             generate_kernel<<<numBlocks, numThreads>>>(devMRGStates, sampleCount, devResults);
         }else if (usePHILOX){
             generate_kernel<<<numBlocks, numThreads>>>(devPHILOXStates, sampleCount, devResults);
     }else {
             generate_kernel<<<numBlocks, numThreads>>>(devStates, sampleCount, devResults);
         }
     }
     cudaDeviceSynchronize();
    ...
    t1 = chrono::high_resolution_clock::now();
     ...
     for(i = 0; i < 50; i++) {
         if (useMRG) {
             generate_uniform_kernel<<<numBlocks, numThreads>>>(devMRGStates, sampleCount, devResults);
         }else if(usePHILOX) {
             generate_uniform_kernel<<<numBlocks, numThreads>>>(devPHILOXStates, sampleCount, devResults);
     }else {
             generate_uniform_kernel<<<numBlocks, numThreads>>>(devStates, sampleCount, devResults);
         }
     }
     cudaDeviceSynchronize();
     t2 = chrono::high_resolution_clock::now();
    ...
    t1 = chrono::high_resolution_clock::now();
     ...
     for(i = 0; i < 50; i++) {
         if (useMRG) {
             generate_normal_kernel<<<numBlocks, numThreads>>>(devMRGStates, sampleCount, devResults);
         }else if(usePHILOX) {
             generate_normal_kernel<<<numBlocks, numThreads>>>(devPHILOXStates, sampleCount, devResults);
     }else {
             generate_normal_kernel<<<numBlocks, numThreads>>>(devStates, sampleCount, devResults);
         }
    }
     cudaDeviceSynchronize();
     t2 = chrono::high_resolution_clock::now();
